...
def parse_params_from_readme(readme_path: pathlib.Path) -> str | None:
    ...
    try:
        with readme_path.open("r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
                if line.startswith("SWARM_COMM_RADIUS"):
                    params["COMM"] = line.split(":", 1)[1].strip()
    ...
    parts = []
    if "COMM" in params:
        parts.append(f"COMM RANGE={params['COMM']}")
    if "DOMAIN" in params:
...
def main():
    ...
    exp_dirs.extend(sorted(BASE_DIR.glob("DISPERSION_experiment*")))
