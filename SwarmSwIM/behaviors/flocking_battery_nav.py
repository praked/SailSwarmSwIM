...
BATTERY_METRICS_FILE = os.environ.get(
    "SWARM_BATTERY_METRICS_FILE",
    f"results/flocking/flocking_battery_metrics_seed_{SEED}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
)

os.makedirs(os.path.dirname(METRICS_FILE) or ".", exist_ok=True)
os.makedirs(os.path.dirname(BATTERY_METRICS_FILE) or ".", exist_ok=True)
...
_battery_metrics_writer = None
def init_battery_metrics_logger():
    global _battery_metrics_writer
    f = open(BATTERY_METRICS_FILE, "w", newline="")
    _battery_metrics_writer = csv.writer(f)
    _battery_metrics_writer.writerow(["time", "agent", "battery_level", "area", "polarisation", "avg_wind_speed", "total_distance"])
...
if __name__ == "__main__":
    ...
    def simulation_callback():
        ...
        if _metrics_writer is not None and (t - metrics_last_time[0]) >= METRICS_PERIOD:
            last_positions = {a.name: np.array(a.pos[:2]) for a in sim.agents}
            total_distance = {a.name: 0.0 for a in sim.agents}
            ...
            for i, a in enumerate(sim.agents):
                ...
                delta = np.linalg.norm(current_pos - last_positions[a.name])
                total_distance[a.name] += delta
                last_positions[a.name] = current_pos.copy()
            ...
            if _battery_metrics_writer is not None:
                for a in sim.agents:
                    battery_level = getattr(getattr(a, "battery", None), "charge", float("nan"))
                    _battery_metrics_writer.writerow([f"{t:.3f}", a.name, f"{float(battery_level):.6f}", f"{area:.6f}", f"{pol:.6f}", f"{avg_wind:.6f}", total_distance[a.name]])
