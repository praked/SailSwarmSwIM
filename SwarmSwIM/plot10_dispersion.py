...
area, avg_wind_speed and arrived agents (loitering).
...
def compute_stats(merged: pd.DataFrame):
    ...
    arrived_cols = [c for c in merged.columns if c.startswith("arrived_")]

    area_vals = merged[area_cols].values
    wind_vals = merged[wind_cols].values
    arrived_vals = merged[arrived_cols].values
    ...
    stats = {
        ...
        "arrived_mean": np.mean(arrived_vals, axis=1),
        "arrived_median": np.round(np.median(arrived_vals, axis=1)),
        "arrived_std": np.std(arrived_vals, axis=1, ddof=1),
    }
    return stats
...
def parse_params_from_csv(csv_path: pathlib.Path) -> str | None:
    ...
    Expected lines (from RunDispersion.sh):
    - SWARM_COMM_RADIUS (communication radius): VALUE
    ...
    try:
        with csv_path.open("r", encoding="utf-8", newline="") as f:
            ...
            if "SWARM_COMM_RADIUS" in row and row["SWARM_COMM_RADIUS"]:
                parts.append(f"COMM RANGE={row['SWARM_COMM_RADIUS']}")
...
def plot_stats(stats, n_runs, out_prefix, exp_name=None, param_info=None):
    ...
    fig.suptitle(f"Dispersion metrics ({n_runs} runs)", fontsize=FONT_SIZE_BASE)
    ...
    if param_info:
        fig.text(0.5, 0.94, param_info, ha="center", va="top", fontsize=FONT_SIZE_BASE)
    ...
    # Add headroom on y-achsis
    y_min, y_max = ax.get_ylim()
    padding = 0.25 * (y_max - y_min)
    ax.set_ylim(y_min, y_max + padding)

    
    # 3) Number of arrived agents
    ax = axes[1]
    ax.plot(time, stats["arrived_median"], color=COLOR_ARRIVED, linewidth=1.4, label="Median")
    ax.fill_between(
        time,
        stats["arrived_mean"] - stats["arrived_std"],
        stats["arrived_mean"] + stats["arrived_std"],
        color=COLOR_ARRIVED,
        alpha=BAND_ALPHA,
        label="±1 SD",
    )
    ax.set_ylabel("Arrived agents")
...
def main():
    ...
    #   - DISPERSION_experiment*
    exp_dirs = []

    exp_dirs.extend(sorted(BASE_DIR.glob("DISPERSION_experiment2")))
    exp_dirs.extend(sorted(BASE_DIR.glob("DISPERSION_experiment3")))
    exp_dirs.extend(sorted(BASE_DIR.glob("DISPERSION_experiment4")))
